...
async def deposit_cmd(client, message: Message):
    ...
    if user.get('wallet',0) < amt:
        return await message.reply('Not enough wallet balance.')
    await users.update_one({'_id': uid}, {'$inc': {'wallet': -amt, 'bank': amt}}, upsert=True)
...
async def withdraw_cmd(client, message: Message):
    ...
    if user.get('bank',0) < amt:
        return await message.reply('Not enough bank balance.')
    await users.update_one({'_id': uid}, {'$inc': {'bank': -amt, 'wallet': amt}}, upsert=True)
